...
class FunctionInfo:
    ...
    category: str                # 函数分类（CORE/RECV/IRQ/INIT/...）
    source_code: str             # 函数源码
    caller_functions: List[str]  # 调用该函数的函数列表
    callee_functions: List[str]  # 该函数调用的函数列表
\end{lstlisting}

需要说明的是，上述数据模型是设计阶段的概念性模型。在实际实现中，为适应 CodeQL 查询结果和 Pydantic 结构化输出的需求，数据模型的字段定义会有所调整。例如，实际的 \texttt{FunctionInfo} 使用 \texttt{location\_line} 而非 \texttt{line\_start/line\_end}，函数类型通过 \texttt{FunctionClassifyResponse} 的 \texttt{Literal} 类型约束来保证类型安全。第四章将详细阐述实际实现中的数据模型定义。
...
详细定义每个类别的判断标准，包括 CORE、RECV、IRQ、INIT、RETURNOK、SKIP、NEEDCHECK、LOOP、NODRIVER 等类别。其中，CORE 类表示 NVIC/OS 内核/VTOR 等关键函数，不允许替换；NEEDCHECK 表示需要进一步检查的函数；SKIP 表示可以直接跳过替换的函数；LOOP 表示包含轮询等待的函数。
...
不同函数类别对应的推荐策略如表~\ref{tab:replace-strategies}所示。需要注意的是，设计阶段的函数分类（如 SEND、CTRL、POLL、DMA）在实际实现中被抽象为更细粒度的类别（如 SKIP、LOOP、NEEDCHECK 等），以更好地处理边界情况和特殊函数。该表格展示了设计阶段的概念性分类与策略映射，实际实现中的分类体系在第四章中详细阐述。
...
\midrule
CORE & N/A & NVIC/OS内核/VTOR关键函数，不允许替换 \\
INIT & Skip & 初始化副作用在仿真中不重要 \\
RECV & Redirect & 从虚拟设备读取数据，保留缓冲区管理 \\
IRQ & Event & 通过事件机制模拟中断 \\
RETURNOK & Skip & 直接返回OK，无副作用操作 \\
SKIP & Skip & 可直接跳过替换的函数 \\
NEEDCHECK & Check & 需要进一步检查的函数 \\
LOOP & Redirect & 轮询等待返回虚拟设备状态 \\
NODRIVER & N/A & 非驱动函数，无需替换 \\
\bottomrule
\end{tabular}
\end{table}
