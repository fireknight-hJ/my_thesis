...
\textbf{模式一：直接地址强转}。将固定地址强制转换为外设结构体指针，常见于寄存器定义的头文件中。识别特征是出现类似 \texttt{((XXX\_TypeDef *)XXX\_BASE)} 的宏或赋值，后续对该指针做字段访问如 \texttt{XXX->REG}。

\textbf{模式二：间接初始化}。通过句柄（handle）/配置表（descriptor/config table）等间接结构，把外设基地址赋值给某个"寄存器块指针成员"，后续通过该成员完成寄存器访问。STM32 HAL 生态中常见成员名为 \texttt{Instance}。识别特征是存在结构体指针类型成员（通常命名为 \texttt{Instance}），该成员被赋值为外设基地址宏、外设实例数组项或配置表中的 \texttt{Instance} 字段，寄存器访问呈现为多级解引用如 \texttt{hxxx->Instance->REG}。

\textbf{模式三：常量指针参数}。函数通过参数接收 MMIO 结构体指针（寄存器块基址），常见于底层驱动/通用外设操作函数。识别特征是函数形参类型为某个寄存器结构体指针，函数体内部通过该参数访问寄存器字段。调用者可以来自模式一（直接传 \texttt{USART1}）或模式二（传 \texttt{huart->Instance}）。
...
本节阐述静态分析模块的具体实现技术。模块采用 CodeQL 静态分析引擎作为底层支撑，CodeQL 通过构建包含语法树、控制流图、数据流图的综合数据库，为语义级代码分析提供了强大的查询能力。

\subsubsection{查询规则设计}

针对三种 MMIO 定位模式，本研究设计了相应的查询规则。查询从 CodeQL 数据库中提取所有符合 MMIO 结构体特征的变量及其初始化表达式。查询文件位于 \texttt{queries/collectors/mmio/} 目录，典型查询会 \texttt{import libraries.MMIOAnalyzer} 并输出"表达式字符串 + 所属函数 + 文件 + 行号 + 类型检查结果"等字段。

对于"间接初始化"和"常量指针参数"模式，采用类似的查询策略，通过追踪赋值语句和函数参数来识别 MMIO 实例。常量基址的追踪利用 CodeQL DataFlow/TaintTracking 将"常量大值表达式（外设基址）"追踪到后续的变量访问/解引用位置，对于"函数第 0 个参数是指针类型，实参是常量大值表达式"的情况，用于定位"把常量基址传给接受结构体指针的驱动函数"的调用点。

\subsubsection{驱动函数定位}

静态分析模块的核心任务是识别固件中的驱动函数，为后续的智能替换提供目标列表。本研究采用简单直接的定位准则：\textbf{凡是包含 MMIO 访问的函数，均定义为驱动函数}。这一定位准则基于硬件交互的直接性、分析的可靠性和替换的必要性三方面考虑。
...
数据流追踪是静态分析的重要环节，其目标是建立 MMIO 访问与函数接口（参数、返回值）之间的关联关系。数据流追踪能够确定参数影响（识别哪些函数参数最终影响了 MMIO 访问）、识别逻辑边界（区分需要保留的业务逻辑与需要替换的硬件操作）、提供定位信息（为替换模块提供精确的语句定位信息，包括需要替换的代码行号、涉及的变量和表达式等）。

本研究利用 CodeQL 提供的数据流分析框架实现 MMIO 数据流追踪。数据流配置定义了从 MMIO 字段访问（源点）到函数参数（汇点）的污点追踪路径，通过污点追踪建立寄存器访问与函数参数之间的关联。

\subsubsection{统一数据模型}

为便于后续模块使用，静态分析结果被组织为统一的数据模型。数据模型采用 Python 数据类（dataclass）定义，支持序列化和反序列化。数据模型包括函数信息模型（包含函数名、文件路径、行号、签名、返回类型、参数列表、分类、源码、调用关系等信息）、MMIO 访问点模型（包含结构体类型、字段名、访问类型、行号、所在函数、上下文代码等）和项目分析结果模型（包含项目路径、CodeQL 数据库路径、所有函数、MMIO 访问点、函数调用图、驱动函数列表、分析时间戳等）。这些数据模型通过 JSON 序列化存储在缓存层，供后续模块查询和使用。
