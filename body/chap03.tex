...
\toprule
\textbf{类别} & \textbf{缩写} & \textbf{描述} & \textbf{典型函数} \\
\midrule
接收类 & RECV & 从硬件/寄存器读取数据 & \texttt{HAL\_UART\_Receive}, \texttt{HAL\_SPI\_Receive} \\
发送类 & SEND & 将数据写入寄存器/外设 & \texttt{HAL\_UART\_Transmit}, \texttt{HAL\_SPI\_Transmit} \\
中断类 & IRQ & 中断处理函数/回调 & \texttt{HAL\_UART\_IRQHandler}, \texttt{USART1\_IRQHandler} \\
初始化类 & INIT & 外设初始化 & \texttt{HAL\_UART\_Init}, \texttt{MX\_USART1\_UART\_Init} \\
控制类 & CTRL & 启停、模式切换 & \texttt{HAL\_UART\_Start}, \texttt{HAL\_UART\_Stop} \\
轮询类 & POLL & 轮询状态位 & \texttt{HAL\_UART\_GetState}, \texttt{\_\_HAL\_UART\_GET\_FLAG} \\
DMA 配置类 & DMA & 配置 DMA 传输 & \texttt{HAL\_UART\_Transmit\_DMA} \\
...
\textbf{策略1：跳过硬件副作用（Skip）}

适用于：初始化类、控制类函数，寄存器写入对仿真无实际影响。

该策略将寄存器写操作替换为无副作用语句，保留函数签名和返回值。对于仿真环境而言，外设初始化和控制操作通常不需要真实执行，因为虚拟外设的状态由仿真器管理。
