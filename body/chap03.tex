...
系统使用大语言模型对驱动函数进行自动分类，通过识别函数与硬件的交互模式，为后续的替换策略选择提供依据。函数分类是整个替换流程的基础，准确的分类能够确保每个函数采用最适合的替换策略，从而保证替换后代码的正确性和仿真环境的稳定性。

分类策略基于对嵌入式驱动函数的硬件交互模式的分析，将函数划分为以下类别：

\textbf{RECV（接收类）}：从硬件寄存器读取数据的函数。这类函数通常负责读取外设的状态、接收数据缓冲区内容等，如 UART 接收函数、SPI 接收函数等。

\textbf{SEND（发送类）}：向硬件寄存器写入数据的函数。这类函数负责向外部设备发送数据，如 UART 发送函数、I2C 发送函数等。

\textbf{IRQ（中断处理类）}：中断处理程序和中断回调函数。这类函数在硬件中断触发时被调用，处理中断事件，如 UART 接收完成中断回调、定时器中断处理函数等。

\textbf{INIT（初始化类）}：外设初始化函数。这类函数负责配置外设的寄存器，使能外设功能，如 UART 初始化、GPIO 初始化等。

\textbf{CTRL（控制类）}：控制函数，如启动、停止、模式切换等操作。这类函数对硬件执行控制命令，如 UART 启用/禁用、DMA 启动等。

\textbf{POLL（轮询类）}：轮询函数，检查硬件状态位。这类函数通常在主循环或等待函数中使用，轮询外设的状态标志，如 UART 传输完成检查、DMA 传输完成检查等。

\textbf{DMA（DMA 配置类）}：DMA 配置函数。这类函数负责配置 DMA 控制器，设置 DMA 传输参数，如 DMA 通道配置、DMA 传输启动等。

\textbf{NODRIVER（纯软件函数）}：纯软件函数，不涉及硬件访问。这类函数不包含 MMIO 访问，在仿真环境中可以直接运行，无需替换。
...
\textbf{策略一：跳过硬件副作用（Skip）}

适用于：初始化类、控制类函数。

该策略将寄存器写操作替换为无副作用语句，保留函数签名和返回值。对于仿真环境而言，外设初始化和控制操作通常不需要真实执行，因为虚拟外设的状态由仿真器管理。

以下代码展示了 Skip 策略的应用。原始函数 \texttt{HAL\_UART\_Enable} 通过设置控制寄存器使能 UART 外设。在仿真环境中，这个操作没有必要，因为虚拟 UART 的状态由仿真器管理。替换后的代码仅保留函数签名，并通过 \texttt{(void)huart;} 语句抑制编译器对未使用参数的警告。
...
\textbf{策略二：剥离硬件代码、保留控制逻辑（Strip）}

适用于：发送类函数。

该策略删除寄存器访问代码，保留状态管理逻辑（如状态变量更新）。该策略确保驱动状态机正确运行，同时避免对不存在硬件的访问。

以下代码展示了 Strip 策略的应用。原始函数 \texttt{HAL\_UART\_Transmit} 通过循环将数据写入 UART 数据寄存器，并轮询等待发送完成。替换后的代码删除了寄存器写操作和轮询等待，保留了驱动状态机的状态更新（\texttt{gState}），将实际的发送操作委托给 \texttt{lcmhal\_uart\_transmit} 函数。
...
\textbf{策略三：替换为仿真 I/O（Redirect）}

适用于：接收/发送类函数。

该策略使用 LCMHal 虚拟设备接口替代硬件访问。LCMHal 是本系统设计的虚拟外设抽象层，为替换后的驱动代码提供与仿真环境交互的标准接口。

以下代码展示了 Redirect 策略的应用。原始函数 \texttt{HAL\_UART\_Receive} 通过轮询状态寄存器等待数据可用，然后从数据寄存器读取数据。替换后的代码直接调用 \texttt{lcmhal\_uart\_receive}，将 I/O 操作重定向到虚拟 UART 设备，避免了硬件等待逻辑。
...
\textbf{策略四：事件驱动模拟（Event）}

适用于：中断回调类函数。

该策略通过事件队列模拟硬件中断触发。在仿真环境中，硬件中断由仿真器生成的事件触发，而非真实的硬件信号。

以下代码展示了 Event 策略的应用。原始函数 \texttt{HAL\_UART\_RxCpltCallback} 是用户实现的中断回调，处理接收完成后的数据。替换后的代码在调用用户回调前，先通过 \texttt{lcmhal\_event\_post} 将事件发布到事件队列，触发后续的事件处理流程。
