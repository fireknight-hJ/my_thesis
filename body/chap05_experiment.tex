...
驱动函数识别是整个替换流程的基础环节，识别的全面性直接影响后续替换的覆盖范围和仿真质量。本节评估系统对驱动函数的自动识别与替换能力，通过统计各测试用例的识别数量、替换数量和迭代修复次数，验证静态分析模块的函数识别覆盖度和LLM的替换代码生成效果。

为了全面评估本文方法的驱动处理能力，本研究统计了每个测试用例的驱动函数识别和替换情况。表~\ref{tab:driver_stats}展示了各测试用例的驱动函数识别与替换统计。
...
\midrule
\textbf{总计} & \textbf{4029} & \textbf{1580} & \textbf{517} \\
\bottomrule
\end{tabular}
&
\begin{tabular}[t]{lccc}
...
fs\_littlefs\_disco\_l475 & 50 & 8 & 2 \\
i2c\_hts221\_disco\_l475 & 37 & 5 & 1 \\
uart\_echo\_disco\_l475 & 35 & 4 & 1 \\
 & & & \\
\midrule
\textbf{平均} & \textbf{115.1} & \textbf{45.1} & \textbf{14.8} \\
\bottomrule
\end{tabular}
\end{tabular}%
}%
...
从统计结果可以看出，本文方法在驱动函数识别和替换方面表现出较高的覆盖度。35个测试用例共识别出4029个驱动函数，平均每个固件115.1个；成功生成1580个替换代码，平均每个固件45.1个。其中517个替换代码经过动态反馈闭环进行了迭代优化，平均每个固件14.8个。需要说明的是，识别出的4029个驱动函数中并非全部需要显式生成替换代码——部分函数经分析确认为纯业务逻辑函数（无需硬件解耦），部分函数可通过模板化策略直接处理（如固定返回成功状态或跳过执行），这些函数的详细分类统计将在第~\ref{sec:classify_stats}节中展开分析。
...
从图~\ref{fig:classification_pie}可以看出，各类驱动函数的占比分布呈现出较为明显的结构性特征，主要体现在以下五个方面：
\textbf{（1）硬件依赖普遍性}：4029 个驱动函数中，1580 个（39.2\%）属于需要显式生成替换函数体的代码生成结果，表明硬件依赖是嵌入式固件中的普遍现象。NODRIVER 类中 812 个函数（20.2\%）被识别为纯业务逻辑函数，避免了对非硬件依赖代码进行不必要替换。PUREDRV 类 978 个函数（24.3\%）由 RETURNOK 与 SKIP 合并而成，采用模板化处理即可在仿真环境稳定运行，因此不计入“显式生成替换代码”口径。上述两类合计1790个，占未生成集合（2449个）的73.1\%，体现了分类策略对“无需复杂替换”的函数具有较好区分能力。
...
\foreach \i/\name/\vA/\vR/\vG in {0/NXP/84.7/26/3.7, 1/STM32/70/6.7/5, 2/Atmel/63/13/9}{
    ...
    \node[above, font=\tiny, anchor=south] at (\xb+\bw/2, \hA) {\vA};
    \node[above, font=\tiny, anchor=south] at (\xb+\bw+\bg+\bw/2, \hR) {\vR};
    \node[above, font=\tiny, anchor=south] at (\xb+2*\bw+2*\bg+\bw/2, \hG) {\vG};
}
...
\foreach \i/\name/\vA/\vR/\vG in {0/NXP/107/39/1, 1/STM32/46/3/1.5, 2/Atmel/79/11/10}{
    \pgfmathsetmacro{\xb}{\xu+0.4+\i*(3*\bw+0.4)}
...
\foreach \i/\name/\vA/\vR/\vG in {0/NXP/140.3/9.8/1, 1/STM32/225/13.5/5.3, 2/Atmel/188/7/1}{
    ...
    \node[above, font=\tiny, anchor=south] at (\xb+\bw/2, \yu+\hA) {\vA};
    \node[above, font=\tiny, anchor=south] at (\xb+\bw+\bg+\bw/2, \yu+\hR) {\vR};
    \node[above, font=\tiny, anchor=south] at (\xb+2*\bw+2*\bg+\bw/2, \yu+\hG) {\vG};
}
...
\foreach \i/\name/\vA/\vR/\vG in {0/NXP/113/2/0, 1/STM32/50/2/1, 2/Atmel/76/25/19}{
    \pgfmathsetmacro{\xb}{\xu+0.4+\i*(3*\bw+0.4)}
    \pgfmathsetmacro{\hA}{\vA/125*\sh}
    \pgfmathsetmacro{\hR}{\vR/125*\sh}
    \pgfmathsetmacro{\hG}{\vG/125*\sh}
...
统计结果表明：35个测试用例中，32个（91.4\%）触发了至少一次替换更新，累计517次。其中155个函数属于"闭环新增覆盖"——这些函数在静态分析阶段未被识别为需要替换（分类为NA或NODRIVER），但在仿真运行过程中暴露出硬件依赖问题，由反馈闭环驱动生成替换代码。若去除动态反馈闭环模块，这155个函数将缺失替换，导致对应的仿真执行路径受阻。

从平台维度分析，Atmel（Zephyr）平台的闭环新增覆盖函数占比最高（39/56，69.6\%），主要原因在于Zephyr采用统一的设备驱动模型（DM），大量硬件操作通过抽象层间接进行，静态分析阶段难以准确识别底层硬件依赖；STM32（RT-Thread）平台次之（52/93，55.9\%），其BSP代码中包含较多的平台抽象层函数和中断处理路径，需要运行时反馈才能精确定位。从外设维度分析，I2C和UART类固件的替换更新均值较高，说明串行通信外设在分析阶段的误分类问题更为突出；ETH类固件的函数分析总数均值最大，但替换更新比例相对较低，表明网络协议栈的驱动函数识别准确率较高。

\subsection{典型案例分析}

本节通过具体案例分析，深入验证动态反馈闭环在分析阶段误分类修正、硬件依赖识别和平台适应性等方面的实际效果。

\textbf{案例1：分析阶段误分类的运行时修正。}在NXP\_I2C\_BareMetal的仿真执行过程中，反馈闭环检测到固件在\texttt{CLOCK\_SetDiv}函数处触发异常循环。该函数定义于NXP MCUXpresso SDK的\texttt{fsl\_clock.h}（第1429行），负责配置时钟分频器，其核心逻辑如代码清单~\ref{lst:clock-setdiv}所示。为说明误分类触发死循环的机制，下面给出关键代码片段。
...
\textbf{案例2：反馈闭环策略边界与CORE函数人工修复。}在LwIP\_StreamingServer的仿真执行过程中，调度器启动阶段触发异常。具体而言，\texttt{vTaskStartScheduler}调用\texttt{xPortStartScheduler}后，该函数首先执行一组Cortex-M中断优先级一致性检查，如代码清单~\ref{lst:port-start-scheduler}所示。相关片段用于展示CORE函数边界下自动修复受限的具体原因。
...
本文选择手动修复而非放开CORE自动替换，基于以下工程考量：CORE函数位于操作系统内核调度关键路径，涉及上下文切换、中断优先级管理、栈帧初始化等与硬件深度耦合的底层机制，贸然由LLM修改可能破坏内核状态一致性，引入难以排查的潜在风险。手动修复策略为：保留\texttt{xPortStartScheduler}函数本体不变，人工编写一份绕过NVIC优先级探测逻辑的stub代码，将其硬编码注入所有基于FreeRTOS的测试用例，后续这些demo不再经过反馈闭环处理该函数。

该案例表明，反馈闭环并非对所有故障都能自动修复，对于涉及操作系统内核关键路径的高风险函数，人工决策仍不可替代。
...
需要说明的是，本文方法与上述两类方法在驱动处理触发机制上存在差异：本文方法采用"静态分析前置+运行时反馈补充"的全量扫描流程，而 Halucinator 与 Para-rehosting 更偏向运行时暴露驱动问题后逐步修复。因此，本节对比重点限定为人工投入强度与在既定测试预算下的实际处理规模，不将三者视为严格同口径的吞吐量对比实验。
...
对于本文方法，基于LLM调用日志统计各固件的词元消耗和处理时长，同时记录人工介入排查和修正的时间。对于Halucinator和Para-rehosting，由熟悉嵌入式开发的工程师执行驱动处理工作，统计实际处理的驱动函数数量和人工投入时间。
...
\midrule
\textbf{平均} & & \textbf{579.2} & \textbf{29.5} & \textbf{652.3} & \textbf{29.4} & \textbf{+73.1} \\
\bottomrule
\end{tabular}
\end{adjustbox}
\end{table}

实验结果表明，在本文方法构建的仿真环境中，AFL++ 能够有效地对固件进行模糊测试。9 个样本中有 7 个样本的基本块覆盖率出现正增长，2 个样本出现轻微负增长；总体平均覆盖基本块数量为 652.3 个，在 24 小时内未发现崩溃。

对比Halucinator，本文方法的平均基本块覆盖数量为652.3个，较Halucinator（579.2个）增加73.1个，提升12.6\%。这一差异的主要原因在于：Halucinator采用手工编写的替换代码，仅处理了运行时暴露的部分驱动函数，未覆盖的驱动函数在模糊测试过程中可能导致固件执行路径中断或异常，从而限制了代码覆盖率。本文方法通过前置的全面静态分析一次性完成驱动替换，使更多硬件相关路径在仿真中可达；结合关键节点验证与运行稳定性结果可见，替换代码在本文设定的仿真目标下能够保持关键行为一致，从而提升了模糊测试的探索深度。
...
总体而言，本文方法在ETH等复杂驱动场景下优势最为突出，在UART等简单外设场景下能够保持与现有方法相当的覆盖率，说明该方法在不同通信外设类型上具有较好的适配能力。
