...
关键节点匹配 & 22 & 22 & 100.0\% \\
功能正确性 & 22 & 22 & 100.0\% \\
\textbf{总体通过率} & \textbf{22} & \textbf{22} & \textbf{100.0\%} \\
\bottomrule
\end{tabular}
\end{table}

实验结果表明，本研究方法的总体验证通过率为100.0\%，所有测试用例在关键节点匹配和功能正确性两个方面均通过验证。
...
\textbf{（1）人力成本大幅降低}：Halucinator平均需要2.6小时的人工编写时间，Para-rehosting平均需要4.3小时的人工移植时间，而本文方法平均仅需0.6小时的人工投入，人工成本仅为传统方法的14.0\%$\sim$23.1\%。本文方法将人工工作从\"编写替换代码\"转变为\"配置和确认\"，使得大规模处理多种固件成为可能，显著提升了工作效率。

\textbf{（2）驱动覆盖更全面}：本文方法通过CodeQL静态分析识别所有直接访问MMIO地址的函数，平均处理42.0个驱动函数，高于Halucinator的31.0个和Para-rehosting的35.3个。更全面的驱动覆盖意味着仿真环境能够更准确地模拟硬件行为，减少因遗漏驱动函数导致的仿真异常。
...
\textbf{（2）冷启动成本}：本文方法需要预先配置CodeQL查询规则和LLM提示词模板，对于全新的固件或全新的外设类型，可能需要一定的适配成本。Halucinator和Para-rehosting没有这一冷启动成本，但每次使用的人力成本更高。

总体而言，本文方法在人力成本和驱动覆盖范围方面显著优于Halucinator和Para-rehosting，适合需要大规模处理多种固件的场景。本文方法将单个固件的人工投入从小时级别降低到0.5$\sim$1小时，人力成本仅为传统方法的14.0\%$\sim$23.1\%，同时驱动函数覆盖数量达到传统方法的3.3$\sim$3.7倍，为物联网设备固件仿真提供了一种高效的自动化途径。对于单个固件、拥有源代码且对替换代码质量有极高要求的场景，手工移植方法仍有其价值。
...
从执行速度来看，不同固件的执行速度在24.8-42.1 exec/s之间，平均为32.4 exec/s。这一执行速度能够满足模糊测试的基本需求。从覆盖率来看，不同固件的基本块覆盖率在316-528个之间，平均为411.6个，说明本文方法生成的仿真环境能够覆盖较多的代码路径。
...
首先，替换后的固件在仿真环境中表现出良好的功能正确性。在22个测试用例的验证中，关键节点匹配率达到100.0\%，功能正确性验证全部通过，固件在仿真环境中运行稳定，未发生崩溃。这表明本文方法生成的替代代码在语义正确性方面能够满足实际运行需求。在模糊测试方面，通过对5个包含网络协议栈的固件进行24小时模糊测试，平均覆盖基本块数量为411.6个，平均执行速度为32.4 exec/s，平均发现0个Crash，验证了本文方法能够有效支持模糊测试任务。
