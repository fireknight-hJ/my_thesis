...
经过筛选和整理，最终构建了包含 35 个固件的测试集，详细信息如表~\ref{tab:firmware}所示。
...
\toprule
\textbf{Demo 名称} & \textbf{识别数} & \textbf{替换数} & \textbf{迭代数} \\
\midrule
...
从统计结果可以看出，本文方法在驱动函数识别和替换方面表现出较高的准确性。35个测试用例共识别出4029个驱动函数，平均每个固件115.1个；成功生成1563个替换代码，平均每个固件44.7个。其中513个替换代码经过动态反馈闭环进行了迭代优化，平均每个固件14.7个。替换代码生成率为38.8\%（1563/4029），未生成替换代码的函数主要包括两类：一是 NODRIVER 类纯业务逻辑函数（812个，占20.2\%），这类函数不直接访问硬件寄存器，无需替换；二是 RETURNOK 类简单查询函数（860个，占21.3\%），这类函数在仿真环境中直接返回成功状态值即可，无需生成替换代码。
...
为了全面评估本文方法的驱动函数分类能力，本研究统计了每个测试用例的驱动函数分类情况（完整统计数据见附录表~\ref{tab:function_categories_stats}）。图~\ref{fig:classification_pie}展示了各类驱动函数的占比分布情况。
...
\fill[cRETURNOK] (0,0) -- (-50.3:2.5) arc (-50.3:-127.1:2.5) -- cycle;
\fill[cNODRIVER] (0,0) -- (-127.1:2.5) arc (-127.1:-199.7:2.5) -- cycle;
\fill[cCORE] (0,0) -- (-199.7:2.5) arc (-199.7:-218.0:2.5) -- cycle;
\fill[cIRQ] (0,0) -- (-218.0:2.5) arc (-218.0:-232.6:2.5) -- cycle;
\fill[cLOOP] (0,0) -- (-232.6:2.5) arc (-232.6:-247.0:2.5) -- cycle;
\fill[cRECV] (0,0) -- (-247.0:2.5) arc (-247.0:-259.5:2.5) -- cycle;
\fill[cSKIP] (0,0) -- (-259.5:2.5) arc (-259.5:-270:2.5) -- cycle;
\draw[white,thick] (0,0) -- (90:2.5);
\draw[white,thick] (0,0) -- (-50.3:2.5);
\draw[white,thick] (0,0) -- (-127.1:2.5);
\draw[white,thick] (0,0) -- (-199.7:2.5);
\draw[white,thick] (0,0) -- (-218.0:2.5);
\draw[white,thick] (0,0) -- (-232.6:2.5);
\draw[white,thick] (0,0) -- (-247.0:2.5);
\draw[white,thick] (0,0) -- (-259.5:2.5);
\node[white,font=\footnotesize,align=center] at (20:1.2) {\textbf{INIT}\\[-1pt]39.0\%};
\node[white,font=\footnotesize,align=center] at (-88.7:1.6) {\textbf{RETURNOK}\\[-1pt]21.3\%};
\node[white,font=\footnotesize,align=center] at (-163.4:1.6) {\textbf{NODRIVER}\\[-1pt]20.2\%};
\node[white,font=\footnotesize,align=center] at (-208.9:1.5) {\textbf{CORE}\\[-1pt]5.1\%};
\draw[thin,cIRQ!70!black] (-225.3:2.6) -- (125:2.9);
\node[anchor=south east,font=\footnotesize] at (125:2.9) {IRQ 4.0\%};
\draw[thin,cLOOP!70!black] (-239.8:2.6) -- (108:2.9);
\node[anchor=south,font=\footnotesize] at (108:2.9) {LOOP 4.0\%};
\draw[thin,cRECV!70!black] (-253.3:2.6) -- (91:2.9);
\node[anchor=south west,font=\footnotesize] at (91:2.9) {RECV 3.4\%};
\draw[thin,cSKIP!70!black] (-264.8:2.6) -- (74:2.9);
\node[anchor=west,font=\footnotesize] at (74:2.9) {SKIP 2.9\%};
\end{tikzpicture}
\caption{驱动函数分类占比分布}
...
\textbf{（1）硬件依赖普遍性}：4029 个驱动函数中，1563 个（38.8\%）生成了替换代码，表明硬件依赖是嵌入式固件普遍存在的特性，验证了驱动函数替换的必要性。NODRIVER 类中 812 个函数（20.2\%）被准确识别为纯业务逻辑函数，避免了对非硬件依赖代码进行不必要的替换。RETURNOK 类 860 个函数（21.3\%）在仿真环境中直接返回成功状态值，无需生成替换代码，体现了分类策略对简单查询函数的合理处理。
...
\midrule
NXP\_FATFS\_BareMetal & main, BOARD\_InitHardware, BOARD\_BootClockRUN \\
NXP\_FATFS\_FreeRTOS & osKernelStart, read\_task, DbgConsole\_Printf \\
NXP\_I2C\_BareMetal & main, BOARD\_InitHardware, LPI2C\_MasterInit \\
NXP\_I2C\_FreeRTOS & osKernelStart, i2c\_task, LPI2C\_MasterTransferNonBlocking \\
NXP\_LwIP\_BareMetal & main, BOARD\_InitHardware, ethernetif\_input, ENET\_GetRxFrame \\
NXP\_LwIP\_FreeRTOS & osKernelStart, eth\_rx\_thread, ENET\_GetRxFrame \\
NXP\_UART\_BareMetal & main, BOARD\_InitHardware, LPUART\_Init \\
NXP\_UART\_FreeRTOS & osKernelStart, uart\_task, LPUART\_TransferSendNonBlocking \\
imxrt1052-nxp-evk & osKernelStart, main, rt\_pin\_write, GPIO\_PinWrite \\
...
\midrule
\multicolumn{2}{l}{\textbf{总计}：14 个测试用例，43 个关键节点，匹配率 100.0\%} \\
\bottomrule
\end{tabular}
\end{adjustbox}
\end{table}

实验结果表明，14个代表性测试用例的关键节点匹配率和替换驱动函数行为验证结果均为正常。总体关键节点匹配率为100.0\%（44/44），表明替换后的固件能够正确执行主要功能路径。

\textbf{（2）替换驱动函数行为验证}

本研究进一步对被调用到的替换驱动函数进行行为验证。根据表~\ref{tab:driver_stats}的统计，35个测试用例共识别出4029个驱动函数，其中1563个生成了替换代码。基于关键节点匹配和替换函数行为分析，14个代表性测试用例的验证结果如表~\ref{tab:function_results}所示。

\begin{table}[htbp]
\centering
\caption{替换驱动函数行为验证结果}
\label{tab:function_results}
\footnotesize
\begin{adjustbox}{max width=\textwidth}
\begin{tabular}{lccc}
\toprule
\textbf{测试样本} & \textbf{功能类型} & \textbf{执行到的替换函数数} & \textbf{验证结果} \\
\midrule
NXP\_FATFS\_BareMetal & \multirow{2}{*}{文件系统（FatFS）} & 10 & 正常 \\
NXP\_FATFS\_FreeRTOS & & 10 & 正常 \\
\midrule
NXP\_I2C\_BareMetal & \multirow{2}{*}{通信外设（I2C）} & 4 & 正常 \\
NXP\_I2C\_FreeRTOS & & 4 & 正常 \\
\midrule
NXP\_LwIP\_BareMetal & \multirow{2}{*}{网络功能（ETH）} & 13 & 正常 \\
NXP\_LwIP\_FreeRTOS & & 15 & 正常 \\
\midrule
NXP\_UART\_BareMetal & \multirow{2}{*}{串口功能（UART）} & 1 & 正常 \\
NXP\_UART\_FreeRTOS & & 6 & 正常 \\
\midrule
imxrt1052-nxp-evk & \multirow{2}{*}{基础功能（GPIO）} & 9 & 正常 \\
imxrt1060-nxp-evk & & 4 & 正常 \\
\midrule
stm32f401-st-nucleo/eth & 网络功能（ETH） & 56 & 正常 \\
stm32f401-st-nucleo/fatfs & 文件系统（FatFS） & 17 & 正常 \\
UART\_Hyperterminal\_IT & 串口功能（UART） & 38 & 正常 \\
LwIP\_HTTP\_Server\_Socket\_RTOS & 网络功能（ETH） & 53 & 正常 \\
\bottomrule
\end{tabular}
\end{adjustbox}
\end{table}

验证结果表明，14个测试用例中被调用到的替换驱动函数均正常返回，未导致仿真崩溃、死循环或异常中断。未被调用到的替换驱动函数主要属于备用初始化路径、异常处理分支等非主流程代码，不影响固件的核心功能运行。

\textbf{（3）案例分析：NXP\_LwIP\_BareMetal 网络数据接收}

为具体展示替换代码在仿真环境中的运行效果，本节以 NXP\_LwIP\_BareMetal（基于 LwIP 协议栈的裸机 HTTP 服务器）为例，展示其完整的仿真执行调用链。该测试用例运行在 NXP i.MX RT1052 平台上，共识别出105个驱动函数，其中61个生成了替换代码。

仿真执行从系统复位开始，依次经过以下关键阶段（摘自函数调用轨迹日志）：

\textbf{阶段A：系统启动与协议栈初始化}（指令周期 1 -- 136623），调用链如清单~\ref{lst:phaseA}所示。
...
\textbf{阶段B：数据接收与协议栈处理}（指令周期 137237 -- 137392），调用链如清单~\ref{lst:phaseB}所示。
...
该案例表明，本文方法生成的替换代码能够正确支撑裸机网络固件从系统启动、协议栈初始化到数据帧接收的完整执行流程，Redirect 策略的仿真注入机制有效弥合了硬件依赖与仿真环境之间的鸿沟。

\subsection{反馈闭环有效性验证}

为验证动态测试反馈闭环模块的有效性，本节通过对比分析评估该模块对仿真成功率的影响。实验思路为：统计各测试用例在反馈闭环阶段触发替换更新的函数数量，分析这些更新对仿真执行的实际贡献。

\subsubsection{替换更新统计}
...
\textbf{案例1：静态分析误分类的运行时修正}

在NXP\_I2C\_BareMetal的仿真执行过程中，反馈闭环检测到固件在\texttt{CLOCK\_SetDiv}函数处触发异常循环。该函数定义于NXP MCUXpresso SDK的\texttt{fsl\_clock.h}（第1429行），负责配置时钟分频器，其核心逻辑如代码清单~\ref{lst:clock-setdiv}所示。

\begin{lstlisting}[style=CStyle,caption={CLOCK\_SetDiv 原函数（NXP\_I2C\_BareMetal，第1429行）},label=lst:clock-setdiv]
...
\textbf{案例1：Zephyr平台MMIO隔离}

atmel/fs\_littlefs\_same54是Atmel平台的Zephyr固件，反馈闭环新增覆盖函数达19个。该固件采用Zephyr统一设备驱动模型（DM），Flash操作通过\texttt{flash\_sam0\_read}等驱动层函数进行，这些函数在静态分析阶段被分类为NA（未识别为驱动函数），未生成替换代码。

如代码清单~\ref{lst:flash-sam0-read}所示，\texttt{flash\_sam0\_read}通过\texttt{memcpy}从物理地址\texttt{CONFIG\_FLASH\_BASE\_ADDRESS}读取数据。在仿真环境中，该地址对应的硬件Flash不存在，直接访问会导致内存错误。

\begin{lstlisting}[style=CStyle,escapeinside={(*@}{@*)},caption={flash\_sam0\_read 原函数与替换代码对比（atmel/fs\_littlefs\_same54）},label=lst:flash-sam0-read]
    ...
    off_t offset, void *data, size_t len)
{
    ...
    if (err != 0) { return err; }
    (*@\lstbg[1pt]{red!20}{memcpy(data, (uint8\_t *)CONFIG\_FLASH\_BASE\_ADDRESS + offset, len);}@*)
    return 0;
}
// replacement:
static int flash_sam0_read(const struct device *dev,
    off_t offset, void *data, size_t len)
{
    ...
    err = flash_sam0_valid_range(offset, len);
    if (err != 0) { return err; }
    (*@\lstbg[1pt]{red!20}{memset(data, 0xFF, len);  // return erased flash state}@*)
    return 0;
}
\end{lstlisting}

反馈闭环生成的替换代码将\texttt{memcpy}替换为\texttt{memset(data, 0xFF, len)}，即返回Flash擦除态（0xFF）而非读取不存在的物理地址。这一替换策略具有明确的语义依据：Flash的默认擦除状态为全1（0xFF），返回该值能够使上层文件系统（LittleFS）正确识别Flash区域为"空白"状态，避免因读取到无效数据（如全0）而触发文件系统的错误恢复路径。反馈闭环对同一固件中的\texttt{flash\_sam0\_erase}（跳过擦除操作，直接返回成功）和\texttt{flash\_sam0\_write}（跳过写入操作）进行了类似的MMIO隔离处理，使LittleFS文件系统在仿真环境中完成了完整的挂载和初始化流程。

该案例验证了反馈闭环对于Zephyr等采用高度抽象驱动模型的平台尤为必要：静态分析难以穿透多层抽象定位底层硬件依赖，而运行时反馈能够根据仿真失败的具体位置和原因，精准生成面向仿真的替换代码。

\subsubsection{有效性分析}

综合上述统计和案例分析，动态反馈闭环模块的必要性体现在以下三个方面：

\textbf{（1）覆盖增量}：154个函数（占总替换覆盖的9.9\%）仅在反馈闭环阶段才被识别并生成替换代码。这些函数主要包括两类：一是静态分析分类为NODRIVER但实际包含硬件依赖的抽象层函数（如Zephyr平台的\texttt{soc\_port\_configure}）；二是静态分析未覆盖但仿真执行中暴露问题的辅助函数。

\textbf{（2）质量修正}：359次替换更新（占总更新次数的70.0\%）属于对已有替换代码的质量修正，包括修复编译错误（签名不匹配、宏展开问题）、调整替换策略（从过度简化恢复为保留关键语义）和补充遗漏的状态管理逻辑。这些修正不增加覆盖数量，但直接决定仿真能否成功执行。

\textbf{（3）平台差异适应性}：不同平台的固件对反馈闭环的依赖程度不同。Atmel（Zephyr）和STM32（RT-Thread）平台的闭环新增覆盖比例较高（62.9\%和55.9\%），NXP（HAL）平台相对较低（17.8\%），这与各平台驱动代码的抽象层次和编码风格一致。

\subsection{本节小结}

本节提出了一种基于关键节点检查的固件仿真功能测试方法，从驱动函数分类评估和功能正确性验证两个维度对本文方法进行了系统评估。

在驱动函数分类评估方面，本文方法在35个测试固件上共识别出4029个驱动函数，自动将其分类为INIT、LOOP、RETURNOK、RECV、IRQ、CORE、SKIP、NODRIVER共8类。其中1563个驱动函数（38.8\%）生成了对应的替换代码，513个经过动态反馈闭环进行了迭代优化，NODRIVER类等非硬件依赖函数被准确排除，验证了基于大语言模型的分类策略能够准确理解驱动函数的语义特征，为后续替换策略的选择提供了可靠的量化依据。

在功能正确性验证方面，本文方法对14个代表性测试用例进行了关键节点匹配和替换驱动函数行为验证。关键节点总体匹配率为100.0\%（43/43），所有被执行到的替换驱动函数行为正常率均为100.0\%，未导致仿真异常。测试覆盖了STM32、NXP和Atmel三种主流平台，涵盖FreeRTOS、RT-Thread、Zephyr和裸机四种操作系统架构以及UART、I2C、ETH、FATFS、Flash等多种外设类型，表明本文方法生成的替换代码能够在仿真环境中正确支持固件的核心功能执行。
...
\toprule
\multirow{2}{*}{\textbf{外设类型（测试用例）}} & \multicolumn{2}{c}{\textbf{Halucinator}} & \multicolumn{2}{c}{\textbf{Para-rehosting}} & \multicolumn{2}{c}{\textbf{本文方法}} \\
...
首先，替换后的固件在仿真环境中表现出良好的功能正确性。在35个测试用例的验证中，共识别出4029个驱动函数，其中1563个（38.8\%）生成了替换代码，513个经过动态反馈闭环进行了迭代优化。14个代表性测试用例的关键节点匹配率达到100.0\%，功能正确性验证全部通过，固件在仿真环境中运行稳定，未发生崩溃。这表明本文方法生成的替换代码在语义正确性方面能够满足实际运行需求。测试覆盖了STM32、NXP、Atmel三种芯片平台和FreeRTOS、RT-Thread、Zephyr、裸机四种操作系统，验证了方法的通用性。在模糊测试方面，通过对7个覆盖ETH、UART、I2C三种外设类型的固件进行24小时模糊测试，平均覆盖基本块数量为1042.7个，平均执行速度为38.5 exec/s，未发现Crash，验证了本文方法能够有效支持多种外设类型的模糊测试任务。
