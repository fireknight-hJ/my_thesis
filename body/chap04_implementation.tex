...
\textbf{代码集成机制}：根据第~\ref{sec:builder-workflow}~节的设计，系统采用"原位替换"策略将替换代码写入工程。对于每个需要替换的驱动函数，系统在目标源文件中保留原始函数定义的快照（用于回滚），然后将替换实现写入函数体。同时，系统在工程中添加弱符号桩函数文件，为仿真注入接口提供链接占位（详见第~\ref{sec:hook-mechanism}~节）。
...
为统一说明仿真执行智能体与反馈闭环的具体实现，本节采用时序视角描述主控调度、故障修复与构建回归的完整交互过程。图~\ref{fig:feedback-loop-sequence}给出了仿真运行主控智能体（\texttt{emulator\_runner\_agent}）、模拟器（\texttt{Emulator}）、修复智能体（\texttt{Fixer}）与构建智能体（\texttt{Builder}）之间的核心调用顺序。
...
该时序对应实现中的一个标准闭环轮次，可分为四个阶段：（1）\textbf{首次仿真}：主控智能体执行 \texttt{EmulateProject()} 发起固件仿真，当返回 \texttt{success:false} 时转入修复流程；（2）\textbf{修复生成}：主控智能体向修复智能体提交故障上下文并触发 \texttt{Fixer()} 分析，修复智能体基于运行日志与调用链返回 \texttt{ReplacementUpdate}；（3）\textbf{代码构建}：主控智能体将修复更新方案交由构建智能体处理，构建智能体应用替换代码并返回 \texttt{BuildOutput}（含编译状态与错误信息）；（4）\textbf{回归仿真}：主控智能体再次执行 \texttt{EmulateProject()} 验证修复效果，若返回 \texttt{success:true} 则输出 \texttt{emulate\_output} 并终止。

从接口设计看，\texttt{ReplacementUpdate} 与 \texttt{BuildOutput} 是闭环稳定运行的关键数据契约：前者约束修复智能体输出的最小必要信息（函数标识、替换实现、修复理由），后者统一表达构建结果（退出码、编译日志、错误摘要）。主控智能体仅通过这两个结构化对象完成跨智能体调度，避免自由文本通信引入的歧义，确保闭环在多轮迭代下仍具备可追踪性与可复现性。
...
本章围绕第三章的设计方案，采用 CodeQL、LangGraph、Unicorn 等技术栈完成了系统的具体实现：基于 CodeQL 实现了三种 MMIO 定位模式的查询规则和数据流追踪；基于 LangGraph 实现了分析、编译修复、仿真执行和闭环修复四个智能体的工作流；基于 Unicorn 构建了仿真平台，通过弱符号桩函数和 Python 处理器实现固件与仿真环境的数据交互。实现结果表明，系统可在大部分环节自动完成从静态分析到动态反馈的流程，必要时由人工进行监督与兜底修正。
