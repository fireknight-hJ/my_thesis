...
为精确定义系统的输入输出关系，以下给出端到端映射的形式化描述。设固件源码工程为 $P$，固件中的函数集合为 $F$，MMIO 结构体实例集合为 $M$，驱动函数类别集合为 $C$，替换策略集合为 $S$。系统的端到端功能可形式化为以下复合映射：
$$\textit{Pipeline} = \textit{Emulate} \circ \textit{Verify} \circ \textit{Replace} \circ \textit{Classify} \circ \textit{StaticAnalysis}: P \to (O, E) \label{eq:pipeline}$$
其中，$\textit{StaticAnalysis}: P \to (D, R, G)$ 从源码中识别驱动函数集合 $D \subseteq F$、MMIO 访问点集合 $R$ 和函数调用图 $G = (F, E_c)$；$\textit{Classify}: D \to C$ 将每个驱动函数映射到其语义类别；$\textit{Replace}: D \times C \to D'$ 对每个驱动函数 $f \in D$ 生成替换实现 $f' \in D'$；$\textit{Verify}: D' \to \{\textit{pass}, \textit{fail}\}$ 对替换实现进行多层验证；$\textit{Emulate}: P' \to (O, E)$ 在仿真环境中运行替换后的工程 $P'$，产生运行输出 $O$ 和运行时错误集合 $E$。系统通过迭代执行"替换$\to$验证$\to$仿真$\to$反馈"闭环，直至 $E = \emptyset$ 或达到最大迭代次数。
...
在静态分析引擎的选型上，需要综合考虑分析能力与工程兼容性。源码级静态分析主要有两种技术路线：基于 LLVM/Clang 的分析和基于 CodeQL 的分析。
...
CodeQL 采用与编译器无关的数据库构建方式：通过在构建过程中拦截编译器调用来捕获源码信息，而非替代原始编译器执行编译。因此，CodeQL 可以直接在 ARM GCC 工具链的构建流程上工作，无需修改固件工程的编译配置。CodeQL 将源码构建为包含 AST、控制流图和数据流图的综合数据库，支持声明式查询语言和全局污点追踪，能够在不运行固件的情况下跨文件追踪 MMIO 基址常量的传播路径，为后续的智能替换提供基础数据。
