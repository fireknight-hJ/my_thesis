...
\textbf{核心思想}：将固件中与硬件强绑定的驱动代码（寄存器读写、HAL/外设库调用、中断与 DMA 配置等）识别出来，用可在仿真环境中执行的替代实现进行等价替换，实现"固件逻辑与硬件依赖解耦"。

\subsection{系统分层架构}

系统采用分层架构设计，按"静态识别 $\to$ 智能替换 $\to$ 构建验证 $\to$ 动态反馈"的链路组织为四个核心模块：
...
\node[layer] (dynamic) {动态测试反馈闭环模块\\(仿真运行 $\to$ 日志采集 $\to$ 错误分析 $\to$ 反馈回流)};
\node[layer, below=0.3cm of dynamic] (replacement) {驱动函数分析、替换与编译模块\\(LLM 分类 $\to$ 替换策略 $\to$ 代码生成 $\to$ 编译集成)};
\node[layer, below=0.3cm of replacement] (analysis) {驱动函数接口静态分析与识别模块\\(CodeQL 分析 $\to$ MMIO 识别 $\to$ 函数分类 $\to$ 数据模型)};
\node[layer, below=0.3cm of analysis] (data) {数据与缓存层\\(分析缓存 | 生成结果 | 编译产物 | 运行日志)};
\end{tikzpicture}
\end{minipage}
\end{center}
...
\textbf{（1）静态分析与识别模块}：对固件源码进行静态分析，定位硬件相关代码，抽取驱动函数、MMIO 访问点、依赖数据结构、调用关系等信息。

\textbf{（2）分析与替换模块}：基于静态分析结果，使用大语言模型对驱动函数进行类型分类与语义保留分析，生成可替代实现，并驱动编译构建。

\textbf{（3）动态测试反馈模块}：在仿真环境中运行替换后的固件，采集运行日志、崩溃信息与覆盖率等指标，将失败样例与错误信息结构化回流，触发重新分析与再生成。

\textbf{（4）数据与缓存层}：贯穿上述链路，统一管理静态分析缓存、生成结果缓存、编译产物与运行日志，支持断点续跑和结果复用。

\subsection{数据流与控制流}

系统的核心数据流包含\textbf{主数据流}和\textbf{反馈数据流}两部分。

\textbf{主数据流}描述正常处理流程：静态分析模块接收源代码，输出函数分析结果；替换编译模块接收分析结果，生成替换代码并完成编译，输出 ELF 文件；动态测试模块接收 ELF 文件，执行仿真并输出运行结果。

\textbf{反馈数据流}描述迭代优化流程：当动态测试检测到运行失败时，将反馈数据传递给替换编译模块，触发重新分析和代码生成，形成迭代闭环，直至运行成功或达到最大迭代次数。
