...
本章通过系统性的实验验证了本文方法的有效性。实验从静态分析准确性、运行正确性、方法对比和模糊测试应用四个方面展开，主要结果如下：
 
首先，静态分析模块在识别固件中的硬件依赖代码方面表现出较高的准确性。通过对20个固件、342个驱动函数的测试，平均准确率达到92.5\%，平均召回率达到90.4\%，F1分数达到91.4\%。对于通信外设（如UART、GPIO）和存储外设（如Flash），识别准确率超过95\%；对于复杂外设（如ETH、定时器），识别准确率略有下降但仍保持在85\%以上。
 
其次，替换后的固件在仿真环境中表现出良好的运行正确性。在20个测试用例的验证中，关键节点匹配率达到100.0\%，功能正确性验证全部通过，固件在仿真环境中运行稳定，未发生崩溃。这表明本文方法生成的替代代码在语义正确性方面能够满足实际运行需求。
 
第三，与现有方法对比显示，本文方法在自动化程度和效率方面具有显著优势。与Halucinator和Para-rehosting相比，本文方法通过LLM自动生成替换代码，将准备时间从小时级别降低到分钟级别（3-5分钟/固件），自动化程度从手工编写提升到完全自动化。同时，本文方法的启动成功率达到100.0\%，高于Halucinator的78\%和Para-rehosting的80\%，编译成功率达到96.7\%，略低于手工方法的100\%但完全可接受。在可扩展性方面，本文方法不依赖源代码，新增平台仅需配置编译工具链和内存映射，具有良好的可扩展性。
 
第四，本文方法能够有效支持后续的模糊测试和漏洞挖掘任务。通过对5个包含网络协议栈的固件进行24小时模糊测试，平均覆盖基本块数量为411.6个，平均执行速度为32.4 exec/s，平均发现3.8个Crash。这些结果表明，在本文方法构建的仿真环境中，AFL++能够有效地对固件进行模糊测试，为漏洞挖掘提供了良好的基础。
 
总体而言，本文方法在静态分析准确性、运行正确性、自动化程度和可扩展性等方面均表现出良好的性能。实验结果充分证明了本文方法的有效性和实用性，为物联网设备固件仿真提供了一种新的技术途径。
