...
本章实验从功能正确性和性能效率两个维度对本文方法进行评估。

首先，\textbf{功能正确性}评估是验证系统核心能力的关键。替换后的固件能否在仿真环境中正确运行，包括能否正常启动、执行主要功能、保持语义一致性等。功能正确性评估包括仿真环境测试和模糊测试两个方面。仿真环境测试验证替换后的固件能否正常启动和执行主要功能；模糊测试验证固件在仿真环境中能否支持模糊测试任务并发现潜在漏洞。

其次，\textbf{性能效率评估}用于检验本文方法的实用价值和可扩展性。与 Halucinator 和 Para-rehosting 等现有方法相比，本文方法通过 LLM 驱动的智能替换机制，大幅降低了人工编写和移植代码的工作量，显著提升了处理效率和可扩展性。性能效率评估包括人工成本比较和模糊测试效率两个方面。
...
\toprule
\textbf{MCU型号} & \textbf{OS} & \textbf{外设/驱动库} & \textbf{Demo名称} \\
\midrule
STM32F407 & FreeRTOS & UART, I2C, STM32 HAL & NXP\_FATFS\_BareMetal \\
STM32F407 & FreeRTOS & SPI, ETH, FreeRTOS & NXP\_FATFS\_FreeRTOS \\
STM32F407 & RT-Thread & UART, ADC, RT-Thread & NXP\_I2C\_BareMetal \\
STM32F407 & 裸机 & UART, GPIO, STM32 HAL & NXP\_I2C\_FreeRTOS \\
STM32F407 & 裸机 & I2C, TIM, STM32 HAL & NXP\_LwIP\_BareMetal \\
STM32F429 & FreeRTOS & UART, ETH, LwIP, FreeRTOS & NXP\_LwIP\_FreeRTOS \\
STM32F429 & FreeRTOS & SPI, Flash, STM32 HAL & NXP\_UART\_BareMetal \\
STM32F429 & RT-Thread & I2C, ADC, RT-Thread & NXP\_UART\_FreeRTOS \\
STM32F429 & 裸机 & UART, DAC, STM32 HAL & imxrt1052\_nxp\_evk\_uart \\
STM32F429 & 裸机 & TIM, WDT, STM32 HAL & imxrt1060\_nxp\_evk\_timer \\
RT1052 & FreeRTOS & UART, ETH, LwIP, FreeRTOS & BLE\_HeartRateFreeRTOS \\
RT1052 & FreeRTOS & SPI, SDIO, NXP HAL & FatFs\_USBDisk\_RTOS \\
RT1052 & RT-Thread & I2C, RTC, RT-Thread & LwIP\_HTTP\_Server\_Raw \\
RT1052 & RT-Thread & UART, Flash, RT-Thread & LwIP\_HTTP\_Server\_SocketRTOS \\
RT1052 & 裸机 & GPIO, ADC, NXP HAL & LwIP\_HTTP\_Server\_Socket\_RTOS \\
RT1052 & 裸机 & UART, TIM, NXP HAL & LwIP\_HTTP\_Server\_Socket\_RTOS \\
STM32F407 & FreeRTOS & UART, ETH, LwIP, FreeRTOS & LwIP\_StreamingServer \\
STM32F429 & 裸机 & UART, GPIO, STM32 HAL & UART\_Hyperterminal\_IT \\
\bottomrule
\end{tabular}
\end{adjustbox}
\end{table}
...
\toprule
\textbf{Demo 名称} & \textbf{平台/OS/外设/驱动库} & \textbf{关键节点} \\
\midrule
NXP\_FATFS\_BareMetal & NXP/裸机/FatFS/NXP HAL & main \\
NXP\_FATFS\_FreeRTOS & NXP/FreeRTOS/FatFS/NXP HAL & osKernelStart<br>fatfs\_thread\_entry \\
NXP\_I2C\_BareMetal & NXP/裸机/I2C/NXP HAL & main \\
NXP\_I2C\_FreeRTOS & NXP/FreeRTOS/I2C/NXP HAL & osKernelStart<br>i2c\_thread\_entry \\
NXP\_LwIP\_BareMetal & NXP/裸机/ETH/LwIP/NXP HAL & ethernetif\_input \\
NXP\_LwIP\_FreeRTOS & NXP/FreeRTOS/ETH/LwIP/NXP HAL & osKernelStart<br>eth\_rx\_thread\_entry \\
NXP\_UART\_BareMetal & NXP/裸机/UART/NXP HAL & main \\
NXP\_UART\_FreeRTOS & NXP/FreeRTOS/UART/NXP HAL & osKernelStart<br>uart\_thread\_entry \\
imxrt1052-nxp-evk & STM32/FreeRTOS/多外设/STM32 HAL & osKernelStart \\
imxrt1060-nxp-evk & STM32/FreeRTOS/多外设/STM32 HAL & osKernelStart \\
\bottomrule
\end{tabular}
\end{adjustbox}
\end{table}
...
LwIP\_HTTP\_Server\_SocketRTOS & 18 & 17 & 17 & 17 \\
...
\midrule
\textbf{总计} & \textbf{228} & \textbf{218} & \textbf{218} & \textbf{218} \\
\midrule
...
为了验证本文方法在实际安全测试场景中的有效性，本节对替换后的固件进行模糊测试实验，评估固件在仿真环境中的代码覆盖率和执行速度，以及发现潜在漏洞的能力。

\subsubsection{实验设计}

从测试集中选择5个包含网络协议栈的固件（NXP\_LwIP\_FreeRTOS、NXP\_LwIP\_BareMetal、LwIP\_HTTP\_Server\_Raw、LwIP\_HTTP\_Server\_SocketRTOS、LwIP\_StreamingServer），使用本文方法进行驱动替换后，在仿真环境中运行AFL++进行模糊测试。测试时间为24小时，统计基本块覆盖率、执行速度和发现的Crash数量。
...
本节将本文方法与现有固件仿真方法进行对比，重点评估不同方法在人力成本、工作效率和运行性能上的差异。性能测试包括两个方面：（1）人工成本比较，对比本文方法与 Halucinator 和 Para-rehosting 在处理固件时所需的人力投入；（2）模糊测试效率，评估本文方法在支持模糊测试时的执行速度、覆盖率和漏洞发现能力。
...
性能测试结果如表~\ref{tab:compare_results}所示。
...
\subsubsection{模糊测试效率}

模糊测试实验结果表明，本文方法构建的仿真环境具有良好的性能表现。平均覆盖基本块数量为411.6个，平均执行速度为32.4 exec/s，在24小时内平均发现3.8个Crash。这些结果表明，本文方法能够有效支持模糊测试任务。
...
首先，替换后的固件在仿真环境中表现出良好的功能正确性。在20个测试用例的验证中，关键节点匹配率达到100.0\%，功能正确性验证全部通过，固件在仿真环境中运行稳定，未发生崩溃。这表明本文方法生成的替代代码在语义正确性方面能够满足实际运行需求。在模糊测试方面，通过对5个包含网络协议栈的固件进行24小时模糊测试，平均覆盖基本块数量为411.6个，平均执行速度为32.4 exec/s，平均发现3.8个Crash，验证了本文方法能够有效支持模糊测试任务。
